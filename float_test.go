...
func TestGetFloat(t *testing.T) {
	...
	cases := []struct {
		...
		{"EnvOK_ValOK_required", valid_env_name, valid_value, true, valid_value_float64, false},
		{"EnvOK_ValOK_optional", valid_env_name, valid_value, false, valid_value_float64, false},
		...
		{"EnvOK_ValTRIM_required", valid_env_name, trimmable_value, true, valid_value_float64, false},
		{"EnvOK_ValTRIM_optional", valid_env_name, trimmable_value, false, valid_value_float64, false},
		...
		{"EnvOK_ValNEG_required", valid_env_name, valid_negative_value, true, valid_negative_value_float64, false},
		{"EnvOK_ValNEG_optional", valid_env_name, valid_negative_value, false, valid_negative_value_float64, false},
	...
	for _, c := range cases {
		...
		t.Run(c.Name, func(t *testing.T) { // can't run in parallel, since there might otherwise be race conditions with setting / unsetting env vars
			...
			res, err := getFloat(valid_env_name, c.Required)
